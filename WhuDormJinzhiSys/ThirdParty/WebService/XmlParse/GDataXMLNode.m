...
- (NSString *)description {
  int nodeType = (xmlNode_ ? (int)xmlNode_->type : -1);
    return [NSString stringWithFormat:@"{type:%d name:%@ xml:\"%@\"}",nodeType, [self name], [self XMLString]];

 // return [NSString stringWithFormat:@"%@ 0x%@: {type:%d name:%@ xml:\"%@\"}",[self class], self, nodeType, [self name], [self XMLString]];
}
